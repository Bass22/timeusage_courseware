...
object TimeUsage {
  ...
  def timeUsageByLifePeriod(): Unit = {
    ...
    finalDf.show()
    val summaryDfTyped = timeUsageSummaryTyped(timeUsageSummary(primaryNeedsColumns, workColumns, otherColumns, initDf))
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      StructField(columnNames.head,StringType, false) :: columnNames.tail.map(
        StructField(_,DoubleType,false)
      )
    )
